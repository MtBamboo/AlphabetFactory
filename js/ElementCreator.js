...
/*　OnClickイベント系*/
function doTabChange(index)
{
	document.getElementById("tabElemental").style.display = index == 0 ? "block" : "none";
	document.getElementById("tabFusionR").style.display = index == 1 ? "block" : "none";
	document.getElementById("tabFissionR").style.display = index == 2 ? "block" : "none";
}
...
function startApp()
{
	...
	//更新開始(setIntervalはID指定で止めない限り永遠に間隔を守って繰り返される)
